...
while(<FI>)
{
    chomp;
    @tmp=split("\t",$_);
    if($tmp[-2]=~/fk/ && exists $sel{$tmp[0]}{$tmp[1]} && exists $anc{$tmp[0]}{$tmp[1]} && $tmp[17]>0)
    {
...
close FI;
open OUT,">zea_sel.ancpsnv.add.sample" or die "$!";
open OUT1,">zea_sel.ancpsnv.rev.sample" or die "$!";
%add=();%rev=();
foreach $chr(1..10)
{
...
close OUT1;
open OUT2,">par_maize.sel.psnv" or die "$!";
