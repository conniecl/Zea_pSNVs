...
while(<LI>)
{
    ...
    {
        if(exists $hit{$tmp[0]}{$tmp[1]})
        {
            push @{$nsys{$hit{$tmp[0]}{$tmp[1]}}},$tmp[1];
        }
    }
}
close LI;
open IN,"<par_maize.hit.psnv" or die "$!";
%hash=();$outregion=0;
while(<IN>)
{
    chomp;
...
close PO;
open OUT,">zea_hit.ancpsnv.rand.sample" or die "$!";
%add=();
foreach $chr(1..10)
{
...
{
    print OUT "$key1\t$add{$key1}\t$pop{$key1}\n";
}
close OUT;
