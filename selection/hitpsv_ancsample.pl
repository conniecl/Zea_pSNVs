...
while(<SE>)
{
    ...
    #{
        $start=$tmp[1]-100000;
        $end=$tmp[2]+100000;
        foreach $i($start..$tmp[1])
        {
            $hit{$tmp[0]}{$i}=join("_",$tmp[0],$start);
        }
        foreach $i($tmp[2]..$end)
        {
            $hit{$tmp[0]}{$i}=join("_",$tmp[0],$tmp[2]);
        }
	#}
}
close SE;
...
while(<FI>)
{
    chomp;
    @tmp=split("\t",$_);
    if($tmp[-2]=~/fk/ && exists $hit{$tmp[0]}{$tmp[1]} && exists $anc{$tmp[0]}{$tmp[1]} && $tmp[17]>0)
    {
        $hash{$tmp[0]}{$tmp[1]}=$tmp[-2];
	#print "$tmp[0]\t$tmp[1]\n"; exit 0;
        $rcount{$hit{$tmp[0]}{$tmp[1]}}+=1;
    }
}
close FI;
open OUT,">zea_hit.ancpsnv.add.sample" or die "$!";
open OUT1,">zea_hit.ancpsnv.rev.sample" or die "$!";
%add=();%rev=();
foreach $chr(1..10)
{
...
close OUT1;
open OUT2,">par_maize.hit.psnv" or die "$!";
